...
from app.services import UserServices, CoinServices, WalletServices, TradeServices, WebsocketServices, JWT_Services
import asyncio
...
@router.post("/login")
async def login_user(email: str, password: str, request: Request, db: AsyncSession = Depends(get_db)):
    return await JWT_Services.login_user(email=email, password=password, request=request, db=db)

@router.get("/users", response_model=List[UserOut])
@JWT_Services.jwt_required
