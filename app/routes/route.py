...
class JWT_Management:
    ...
    async def login(email, password, db: AsyncSession):
        user = await db.exec(select(User).where(User.email==email, User.password==password))
        user = user.first()
        if user:
    ...
    def require_jwt(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if user:
                ...
                return JSONResponse(status_code=403, content={"message": "You are not authenticated to use this route..."})
