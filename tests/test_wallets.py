...
async def test_get_wallet(client, user, access_token):
    response = await client.get(
        f"/wallet/?user_id={user.get('id')}",
        headers = {"access_token": access_token}
        )
    assert response.json()['name'] == user.get("first_name") + " " + user.get("last_name")
...
@pytest.mark.anyio
async def test_get_coin_list(client, coin):
    response = await client.get("/coins/")
    assert response.json()[0]['id'] == coin.get('id')
    assert response.json()[0]['name'] == coin.get('name')
    assert response.json()[0]['symbol'] == coin.get('symbol')
    assert response.status_code == 200
...
@pytest.mark.anyio
async def test_withdraw_from_wallet_more_than_availible_amount(deposit, client, access_token):
    response = await client.post(
        f"/withdraw/?user_id={deposit.get('user_id')}&coin_id={deposit.get('coin_id')}&amount={deposit.get('amount')+10}",
        headers = {"access_token": access_token}
    )
